...
describe('GET /api/v1/jeu/jouer/:id', () => {
    for (let i = 0; i < 20; i++) {
        it(`devrait répondre avec un appel réussi pour ${testNom1}`, async () => {
            ...
            expect(resultat.v3).toBeWithin(1, 7); // ✅ ajout pour le 3e dé
            expect(resultat.somme).toBe(resultat.v1 + resultat.v2 + resultat.v3);
            expect(resultat.nom).toBe(testNom1);
        });
    }

    it(`devrait répondre avec une erreur 404 si le joueur ${testNom2} n'existe pas`, async () => {
